...
export const useEmissionsChart = () => {
  ...
  useEffect(() => {
    ...
    // Filtra datos de la compañía seleccionada y tipos visibles
    const companyData = allData.filter((e) => e.companyId === selectedCompanyId);
    const visibleData = companyData.filter((e) => visibleTypes.has(e.pollutionType));
  
    // Extrae y ordena fechas únicas
    const dates = [...new Set(visibleData.map((e) => e.date.split("T")[0]))].sort(
      (a, b) => a.localeCompare(b)
    );
  
    setLabels(dates);
  }, [selectedCompanyId, allData, visibleTypes]);
